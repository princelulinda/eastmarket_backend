...
export const PostRegisterStartSchema = z.object({
  email: z.string().email(),
}).strict()

type PostBody = z.infer<typeof PostRegisterStartSchema>
...
export const POST = async (req: MedusaRequest<PostBody>, res: MedusaResponse) => {
  ...
  if (existing.length > 0) {
    throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "Un compte existe déjà avec cet email.")
  }
